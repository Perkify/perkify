...
const allPerks = require("./perks.json");

// rapyd credentials
const rapydSecretKey = "9ddd95bd2a2beb670c8297afec7fdea3a8cca2f64488d30e394aeebf5208d9c78e4f9c0a6ac0d5c8";
...
const sanitizeEmails = (emails) => {
  return emails.map((email)=>validator.normalizeEmail(email, emailNormalizationOptions));
};
...
const registerUserValidators = [
  body("email").isEmail().normalizeEmail(emailNormalizationOptions).custom(validateUserEmail),
  body("firstName").not().isEmpty(),
  body("lastName").not().isEmpty(),
  body("dob").isDate(),
];
...
app.post("/registerAdminAndBusiness", registerAdminAndBusinessValidators, registerAdminAndBusiness);
